...
def read_base_scenario():
    ...
    """
    yaml_path = resolve_paths().execution_config
    try:
...
class SecondaryTechsUpdater:
        ...
        for scenario in self.scenarios:
            ...
            try:
                ...
                for country_key, row_idx in existing_countries.items():
                    ...
                    # OSTRAM data uses 3-char ISO codes, so try both the key and its ISO3 prefix
                    iso3 = country_key[:3]
                ...
                # OSTRAM generation data uses 3-char ISO codes (e.g., "IND"), while
            ...
            if year <= base_year:
                ...
                if year >= sorted_target_years[-1]:
                    ...
                    if interpolation == 'linear':
                        ...
                        growth_rates = self.ostram_config.get('demand_growth_rates', {})
                        demand_growth_rate = lookup_growth_rate(growth_rates, country_code)  # Default 2%
    ...
    def read_osemosys_defaults(self):
        ...
        yaml_path = resolve_paths().compilation_resources / 'conversion_format.yaml'
    ...
    def update_activity_limits(self, all_years):
        ...
        for scenario in self.scenarios:
            ...
            try:
                ...
                # Uses pwr_country_key to get region-aware keys for multi-region countries
...
def main():
    try:
        # Paths
        paths = resolve_paths()
        runtime = paths.stage_workspace("preparation", create=True)
        sources = paths.secondary_technology_inputs
        editor_path = runtime / "Secondary_Techs_Editor.xlsx"
        base_path = paths.a1_outputs
        ostram_file_path = sources / "OSTRAM - Installed Capacity by Source - Annual.xlsx"
        shares_file_path = sources / "Shares_PET_OIL_Split.xlsx"
        generation_file_path = sources / "OSTRAM - Electric Generation by Source - Annual.xlsx"
        shares_total_file_path = sources / "Shares_Power_Generation_Technologies.xlsx"
        trade_balance_file_path = runtime / "secondary_techs" / "flujos_energia_estimados_optimizacion.xlsx"
