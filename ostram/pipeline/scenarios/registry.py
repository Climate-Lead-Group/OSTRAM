...
class RootScenario:
    ...
    restriction_dependencies: tuple[str, ...] = ()
...
class ScenarioRegistry:
    ...
    def required_roots(self, selected: Iterable[str]) -> tuple[str, ...]:
        ...
        Declared ``restrictions`` dependencies are expanded transitively and
        de-duplicated, so a root whose Restrictions rows are generated by
        another root is always materialized after that prerequisite.  The
        expansion widens only materialization and snapshot checking; callers
        keep their exact downstream selection from :meth:`select`.
        """

        required: set[str] = set()
        derived = self.derived_by_name
        roots = self.roots_by_name
        ...
        while frontier:
            for prerequisite in roots[frontier.pop()].restriction_dependencies:
...
def load_registry(
    ...
    *,
    ...
    for entry in raw.get("root_scenarios", ()):
        ...
        restriction_dependencies: list[str] = []
        for dependency in entry.get("dependencies", ()):
            if dependency.get("type") == "restrictions":
                scenario = dependency.get("scenario")
                if not scenario or not str(scenario).strip():
                    raise ValueError(
                        f"restriction dependency for {entry.get('name')} "
                        "must name a scenario"
                    )
                restriction_dependencies.append(str(scenario).strip())
                continue
        ...
        roots.append(
            RootScenario(
                ...
                restriction_dependencies=tuple(restriction_dependencies),
            )
        )
...
def _validate_registry(
    ...
    *,
    ...
    # Restriction dependencies must form a strict forward chain in the
    # Control root order: every prerequisite is materialized before its
    # dependent.  The ordering rule structurally excludes self-referential
    # and cyclic declarations.
    root_order = {name: index for index, name in enumerate(root_names)}
    for root in registry.roots:
        repeated = _duplicates(list(root.restriction_dependencies))
        if repeated:
            raise ValueError(
                f"duplicate restriction dependencies for {root.name}: "
                f"{repeated}"
            )
        for prerequisite in root.restriction_dependencies:
            if prerequisite not in root_set:
                raise ValueError(
                    f"{root.name} declares a restriction dependency on "
                    f"unknown root {prerequisite}"
                )
            if prerequisite == root.name:
                raise ValueError(
                    f"{root.name} declares a restriction dependency on itself"
                )
            if root_order[prerequisite] >= root_order[root.name]:
                raise ValueError(
                    f"restriction dependency {prerequisite} must precede "
                    f"{root.name} in the Control root order; cyclic "
